# this script strictly allow X in {1, 2} only 
# ***cityIndicator is this script is different from fixed effect model***
# theta0 is 'intercept' of the outcome model
# theta is vector of length n.city
# cityIndicator is matrix of n*n.city, j-th binary column indicating for belonging to city (j)
# trtIndicator, the first column indicate being in control group, the second column indicate being in treatment group
...
gibbs_sample <- function(Y, X, W1, W2, V, SPL, 
  ...
  ########################## parse data ##########################
  cityIndicator <- matrix(data=0, nrow=n, ncol=n.city)
  cityIndicator[cbind(1:n, SPL)] <- 1
  
  ## colSums(cityIndicator) is the same as unname(v.city)
  
  ## first columns for X=1, second for X=2, etc 
  trtIndicator <- matrix(data=0, nrow=n, ncol=n.X)
  trtIndicator[cbind(1:n, X)] <- 1
  ...
  ### Outcome ###
  theta0 <- matrix(NA, nrow=iter, ncol=1)
  theta0[1] <- mean(Y, na.rm=T)  
  
  mu.theta <- matrix(NA, nrow=iter, ncol=1)
  mu.theta[1] <-mean(Y, na.rm=T) 
  
  theta <- matrix(NA, nrow=iter, ncol=n.city)
  theta[1,] <- mean(Y, na.rm=T)
  
  sig2.theta <- matrix(NA, nrow=iter, ncol=1)
  sig2.theta[1] <- .0001 # large enough 
  ...
  effect <- effect.M2 <- effect.higher <- 0
  ...
  ####################################### init from const effect model output ######################## 
  if("theta.1" %in% names(init)) theta0[1] <-init$theta.1
  if("theta.2" %in% names(init)) {theta[1,] <- mu.theta[1] <- init$theta.2}
  if("omega" %in% names(init)) omega[1] <- init$omega
  if("sigma2.w1" %in% names(init)) sigma2.w1[1] <- init$sigma2.w1
  if("alpha.w1" %in% names(init)) alpha.ww1[1,] <- c(init$alpha.w1, init$alpha.w1u)
  if("sigma2.w2" %in% names(init)) sigma2.w2[1] <- init$sigma2.w2
  if("alpha.w2" %in% names(init)) alpha.ww2[1,] <- c(init$alpha.w2, init$alpha.w2u)
  if("sigma2.v" %in% names(init)) sigma2.v[1] <- init$sigma2.v
  if("alpha.v" %in% names(init)) alpha.vv[1,] <- c(init$alpha.v, init$alpha.vu)
  if("sigma2.y" %in% names(init)) sigma2.y[1] <- init$sigma2.y
  if("alpha.yu" %in% names(init)) alpha.yu[1,] <- init$alpha.yu
  if("eta.1" %in% names(init)) {
    if(n.u==1) {
      eta[1,] <- c(init$eta.1, init$eta.2)
    } else if(n.u==2) {
      eta[1,] <- c(init$eta.1, init$eta.2, init$eta.3) }}
  # if("sig2.eta" %in% names(init)) sig2.eta[1] <- init$sig2.eta
  if("sig2.Z" %in% names(init)) sig2.Z[1] <- init$sig2.Z
  if("sigma2.u" %in% names(init)) sigma2.u[1, ] <- init$sigma2.u
  
  if(!knowU) {
    if("UU.mean" %in% names(init)) {
      UU.full <- init$UU.mean
      UU <- as.list(split.data.frame(UU.full, rep(1:n.city, ncol.B))) 
    }
    if("BU.mean" %in% names(init)) BUU <- init$BU.mean
  }
  if(!knowZ) {
    if("ZZ.mean" %in% names(init)) ZZ[1, ] <- init$ZZ.mean
    if("BZ.mean" %in% names(init)) BZZ <- init$BZ.mean
  }
  ###################################################################################################### 
  ...
  for (i in 2:iter) {
    ...
    if(i%%1000==0) {
      print(paste(i, timestamp()))
      # print(plot(mu.theta-theta0))
    }
    ...
    if (!knowU) {
      ...
      for (j in 1:n.u) {
        ...
        ## P(Y|U) 
        yTEMP <- theta0[i-1]*trtIndicator[,1] + (cityIndicator %*% oneCol(theta[i-1, ]))*trtIndicator[,2] + omega[i-1]*BZZ
                 # control effect * being in control group  
                                        # cityIndicator * city-level effect * being in treatment group 
        ...
        ## all we want is avoid using test data to determine the model 
        ## so simply mask test data with zero
    ...
    ## could add if(no.proxy) alpha.ww[] <- alpha.ww.cand[] <- 0 
    if(n.u == 1) {
      ############ W1 ############
      ### intercept 
    ...
    # status theta0[i-1] alpha.yu[i-1] omega[i-1] theta[i-1]
    YPart <- Y - theta0[i-1]*trtIndicator[,1] - BUU %*% alpha.yu[i-1,] - omega[i-1]*c(BZZ)
    ...
    for(kk in 1:n.city) {
      ...
      theta.cand <- theta[i-1, kk] + rnorm(1, 0, MH.theta/sqrt(idy1-idy0+1))
      
      lp.prev <- sum(log.dnorm(YPart[idy0:idy1] - theta[i-1, kk]*trtIndicator[idy0:idy1,2], 0, sigma2.y[i-1])) - 0.5*(theta[i-1, kk] - mu.theta[i-1])^2/sig2.theta[i-1] 
      lp.cand <- sum(log.dnorm(YPart[idy0:idy1] - theta.cand    *trtIndicator[idy0:idy1,2], 0, sigma2.y[i-1])) - 0.5*(theta.cand     - mu.theta[i-1])^2/sig2.theta[i-1] 
      
      accept <- (min(1, exp(lp.cand-lp.prev)) > runif(1))
      if(i>100) {
        if(accept)
          theta[i, kk] <- theta.cand
      }
      
      temp <- updateMH(accept, acc.theta, att.theta, MH.theta, i, nburn)
      acc.theta <- temp[[1]]; att.theta <- temp[[2]]; MH.theta <- temp[[3]]
      
      idy0 <- idy1 + 1
    } # end for(j in 1:n.city)
    
    sig2.theta.cand <- min(rlnorm(1, meanlog = log(sig2.theta[i-1]), sdlog = MH.sig2.theta), 10^4) # set maximum as 10^4
    lp.prev <- sum(log.dnorm(theta[i, ] - mu.theta[i-1], 0, sig2.theta[i-1])) + dgamma(1/sig2.theta[i-1], a, b, log=T)
    lp.cand <- sum(log.dnorm(theta[i, ] - mu.theta[i-1], 0, sig2.theta.cand)) + dgamma(1/sig2.theta.cand, a, b, log=T)
    accept <- (min(1, exp(lp.cand-lp.prev)) > runif(1))
    if(i<100) {
      sig2.theta[i] <- sig2.theta[i-1]
    } else {
      if(accept) {sig2.theta[i] <- sig2.theta.cand} else {sig2.theta[i] <- sig2.theta[i-1]}
    }
    temp <- updateMH(accept, acc.sig2.theta, att.sig2.theta, MH.sig2.theta, i, nburn)
    acc.sig2.theta <- temp[[1]]; att.sig2.theta <- temp[[2]]; MH.sig2.theta <- temp[[3]]
    
    
    mu.theta.cand <- mu.theta[i-1] + rnorm(1, 0, MH.mu.theta)
    lp.prev <- sum(log.dnorm(theta[i, ] - mu.theta[i-1], 0, sig2.theta[i])) + log.dnorm(mu.theta[i-1], 0, sd=10^2)
    lp.cand <- sum(log.dnorm(theta[i, ] - mu.theta.cand, 0, sig2.theta[i])) + log.dnorm(mu.theta.cand, 0, sd=10^2)
    accept <- (min(1, exp(lp.cand-lp.prev)) > runif(1))
    if(accept) {mu.theta[i] <- mu.theta.cand} else {mu.theta[i] <- mu.theta[i-1]}
    temp <- updateMH(accept, acc.mu.theta, att.mu.theta, MH.mu.theta, i, nburn)
    acc.mu.theta <- temp[[1]]; att.mu.theta <- temp[[2]]; MH.mu.theta <- temp[[3]]
    
    # status theta0[i-1] alpha.yu[i-1] omega[i-1] theta[i]
    YPart <- Y - (cityIndicator %*% oneCol(theta[i, ]))*trtIndicator[,2] - omega[i-1]*c(BZZ)
    
    XXX <- cbind(trtIndicator[,1], BUU)
    
    txx <- t(XXX[!mask,])%*%XXX[!mask,]
    txy <- t(XXX[!mask,])%*%YPart[!mask]
    p1 <- 1 + n.u
    ...
    if(i<100) {
      theta0[i] <- theta0[i-1]
      alpha.yu[i, ] <- alpha.yu[i-1, ] 
    } else {
      theta0[i] <- beta1[1]
      alpha.yu[i, ] <- beta1[2:(1+n.u)]
    }
    
    if(no.proxy) alpha.yu[i, ] <- 0 # BUU are all zero, but in such case
    
    ################ update spline part in standard regression ################
    # status theta0[i] alpha.yu[i] omega[i-1] theta[i]
    
    YPart <- Y - theta0[i]*trtIndicator[,1] - BUU %*% alpha.yu[i,] - (cityIndicator %*% oneCol(theta[i, ]))*trtIndicator[,2]
    ...
    if (!knowZ) {
      ...
      for(j in 1:n.city) {
        ...
        yk <- Y[idy0:idy1]
        buu <- BUU[idy0:idy1, ] 
        trtIndicatork <- trtIndicator[idy0:idy1, ]
        ...
        if(idy1-idy0==0) { # only one region in one city 
          yTEMP <- theta0[i]*trtIndicatork[1] + theta[i, j]*trtIndicatork[2] + buu %*% oneCol(alpha.yu[i, ])
        } else {
          yTEMP <- theta0[i]*trtIndicatork[,1] + theta[i, j]*trtIndicatork[,2] + buu %*% oneCol(alpha.yu[i, ])
        }
    ...
    if(i>nburn){
      ...
      effect.i <- theta[i,] - theta0[i]
      
      effect <-        effect        + effect.i    /(iter-nburn)
      effect.M2 <-     effect.M2     + effect.i^2  /(iter-nburn)
      effect.higher <- effect.higher + (effect.i>0)/(iter-nburn)
      ...
      BZ.mean <- BZ.mean + BZZ/(iter-nburn)
      BZ.M2   <- BZ.M2 + BZZ^2/(iter-nburn)
  ...
  out <- list(iteration=keep, 
              ...
              theta=theta[keep,], # for test
              sig2.theta=sig2.theta[keep],
  ...
  return(list(out=as.data.frame(out), 
              ...
              effect=effect, effect.var=effect.var, effect.higher=effect.higher)) 
}
