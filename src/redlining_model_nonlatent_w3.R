...
source("./src/helper.R")
source("./src/bayes_mcmc_nolatent_3w.R")
source("./src/spline.R")
library(tidyr)
library(caret)

cases <- rbind(as.matrix(expand.grid(0:9/10, # ratio.uy
                                     c(1,2) # pm25 or no2 
                                     ))) 
...
iter=30*10^4; nburn=15*10^4; nthin=10; nc=1 # 4min for 10^4 in local 

## Test 
# iter=2000; nburn=1000; nthin=1
# iter=30; nburn=15; nthin=1
# args=2

############################# load redlining data ############################# 
splineBy <- readRDS(paste0("data/cleaned/RedliningSplineMat_R", ratio.uy*100, ".rds"))
...
###########################################################################
######################### Case: real data analysis ########################
###########################################################################
list2env(lapply(as.list(dt[, c("Y", "W1", "W2", "V", "X", "SPL")]), as.vector), .GlobalEnv)

## set up output 
output.folder <- paste0("redlining_mcmc_nonlatent_w3") 
list2env(output.create(id=output.folder, id2=""), globalenv())
output.file <- file.path(output.folder, paste0(outcome, "_3w_s", ratio.uy*100, ".Rdata"))
...
input <- output <- "output/redlining_mcmc_nonlatent_w3"
u <- 0; ratio.u <- 0

for (outcome in c("pm25", "no2")) {
  tb1 <- c()
  for (ratio.uy in c(0,.1,.15,.2,.25,.3,.35,.4,.45,.5,.6,.7,.8,.9)) {
    file_path <- file.path(input, paste0(outcome, "_3w_s", ratio.uy*100, ".Rdata"))
