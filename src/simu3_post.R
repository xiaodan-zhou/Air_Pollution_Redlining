...
input <- output <- "output/simu3_latent_u1w3" 
ratio.u <- Inf 
u <- 1
for (outcome in c("Y")) {
  tb1 <- c()
...
################################################################
################################################################
input <- output <- "output/simu3_nonlatent_w3"
u <- 0; ratio.u <- 0
...
################################################################
################################################################
input <- output <- "output/simu3_no_adjustment"
u <- 0; ratio.u <- 0
...
###################### estimate ################################
################################################################
dir.create("output/simu3_randUZ_plots", recursive = TRUE)

## get the true effect

# load("output/simu3_randUZ_data/data1.RData") forget to save 
true_effect <- .2 # as.numeric(argg["theta.2"]-argg["theta.1"])

t2 <- readRDS("output/simu3_nonlatent_w3/w3.rds")
t2$method <- "Outcome Adjustment"
t2$withZ <- "Yes"; t2$withZ[t2$ratio.uy == 0] <- "No"

t4 <- readRDS("output/simu3_latent_u1w3/u1w3.rds")
t4 <- t4[t4$ratio.u==Inf,]
t4$method <- "Latent model"
t4$withZ <- "Yes"; t4$withZ[t4$ratio.uy == 0] <- "No"

t1 <- readRDS("output/simu3_no_adjustment/noproxy.rds")
...
saveRDS(tb2, file = "output/simu3_latent_u1w3/combined.rds")
